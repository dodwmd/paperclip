...
import { agents as agentsTable, companies, heartbeatRuns, instanceMcpCatalog, issues as issuesTable } from "@paperclipai/db";
...
export function agentRoutes(db: Db) {
  ...
  router.post("/heartbeat-runs/:runId/acknowledge", async (req, res) => {
    assertBoard(req);
    const runId = req.params.runId as string;
    const run = await heartbeat.getRun(runId);
    if (!run) {
  ...
  router.post("/agents/:id/sync-persona", async (req, res) => {
    ...
    const actor = getActorInfo(req);
    const result = await syncAgentPersona(
      db,
      { id: existing.id, companyId: existing.companyId, personaGitUrl: existing.personaGitUrl },
      actor,
    );
